...
wrapper_add_repository_entry() {
    repository_entry_name_to_add=$1
    repository_entry_path_to_add=$2
    if repository_entry_existing_path=$(repository_path_for_name "$repository_entry_name_to_add"); then
        [ "$repository_entry_existing_path" = "$repository_entry_path_to_add" ] \
            || die "repository name is already attached to a different path: ${repository_entry_name_to_add}"
        return 0
    fi
    if repository_path_is_configured "$repository_entry_path_to_add"; then
        die "repository is already attached with a different name: ${repository_entry_path_to_add}"
    fi
    atomic_write "$CONFIG_FILE" awk \
        -v name="$repository_entry_name_to_add" \
        -v path="$repository_entry_path_to_add" '
        { print }
        END {
            print ""
...
wrapper_ensure_repository_links() {
    [ -n "${REPOSITORY_PATHS:-}" ] || return 0
    ensure_directory "$REPO_LINK"
    while IFS= read -r repository_path; do
        [ -n "$repository_path" ] || continue
        repository_link_path_value=$(repository_link_path "$repository_path")
        wrapper_ensure_link "$repository_link_path_value" "$repository_path" "repository"
    done <<EOF
$REPOSITORY_PATHS
EOF
}
...
$REPOSITORY_PATHS
EOF
        [ "$missing_repository" -eq 1 ] || wrapper_ensure_repository_links
    else
